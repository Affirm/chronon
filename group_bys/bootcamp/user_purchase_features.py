...
# Import the staging query that creates the Iceberg table from S3
import sys
import os
sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
from staging_queries.bootcamp.purchases_from_s3 import v1 as purchases_staging_query

# Define the source using the Iceberg table created by the staging query
# This automatically reads from bootcamp.purchases_from_s3 (the staging query output)
source = StagingQueryEventSource(
    staging_query=purchases_staging_query,
    query=Query(
